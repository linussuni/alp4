...
int entnehmen(void *t_args)
{
    ...
    // entnehme gewuenscht viele Produkte
    for (int x = 0; x < args->num_takeout; x++)
    {
...
{
    // initialisiere Daten
    int num_threads_consumers, num_threads_producers, num_takeout;
    int rc;
    long t;
    ...
    printf("How many items should be consumed at a time? :");
    scanf("%d", &num_takeout);

    // handle Input
    if (num_threads_consumers > 0 && num_threads_producers > 0 && num_takeout > 0 && num_takeout <= NUM_PLACES)
    {
        ;
    }
    else
    ...
    printf("Number of items to consume at a time: >%d< \n", num_takeout);
    ...
    for (t = 0; t < num_threads_consumers; t++)
    {
        ...
        t_args->threadid = t;
        t_args->num_takeout = num_takeout;
